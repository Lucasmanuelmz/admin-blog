...
describe('Criando artigo com Rich Text Editor', () => {
  ...
  it('deve renderizar todos os campos do formulário', () => {
    ...
    expect(screen.getByLabelText(/Adicione uma imagem ao artigo/i)).toBeInTheDocument();
    expect(screen.getByText(/Conteúdo/i)).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /Publicar Artigo/i })).toBeInTheDocument();
  });
  ...
  it('deve exibir o nome do arquivo ao fazer upload de imagem', async () => {
    ...
    const file = new File(['imagem'], 'imagem-teste.png', { type: 'image/png' });
    const imageInput = screen.getByLabelText(/Adicione uma imagem ao artigo/i);

    await userEvent.upload(imageInput, file);

    expect(screen.getByText(/Imagem selecionada:/i)).toHaveTextContent('Imagem selecionada: imagem-teste.png');
  });
});
